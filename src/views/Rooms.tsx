        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredRooms.map((room, i) => (
            <motion.div
              ...
              layout
            >
              <Card className="overflow-hidden border-none shadow-md hover:shadow-xl transition-all duration-300 group h-full flex flex-col bg-brand-cream-dark">
                ...
                <CardContent className="p-6 flex flex-col grow">
                  <CardTitle className="mb-3">{room.name}</CardTitle>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-brand-brown/10">
                    <div className="flex gap-2 text-brand-brown/60 text-sm">
                      <span>{room.size}</span>
                      <span>•</span>
                      <span>{room.bedType}</span>
                    </div>
                    <Link href={`/rooms/${room.id}`}>
                      <Button variant="ghost" className="text-brand-red hover:text-brand-red-hover hover:bg-brand-red/10">
                        Details
                      </Button>
                    </Link>
                  </div>
